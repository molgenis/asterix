...
plot_cnv_variant_intensities <- function(
  data, x_variable, y_variable, col_variable, var, cluster_col = NULL, cluster_centers = NULL) {

  variant_data_frame_missingness <- data %>% filter(
    variant == var)
  ...
  n_missing_samples <- sum(is.na(variant_data_frame_missingness[, x_variable, drop = T]))

  genotype_frequencies <- variant_data_frame %>%
    mutate(ALL_N = n()) %>%
  ...
  if (col_variable == "genotype") {
    ...
    this_plot <- variant_data_frame %>%
      ...
      geom_mark_ellipse(aes(label = genotype, filter = !is.na(genotype)),
                        expand = 0.02) +
      scale_colour_manual(
        values = plasma(length(genotype_labels), alpha = 1, begin = 0.1, end = 0.9, direction = 1), name = "Genotype",
        breaks = unname(genotype_labels),
        na.value = "grey50") +
      scale_size_manual(values = c("incomplete" = 1, "complete" = 0.75)) +
      scale_shape_manual(values = c("incomplete" = 21, "complete" = 16)) +
      geom_richtext(
        data = variant_data,
    ...
    this_plot <- variant_data_frame %>%
      ...
      # geom_mark_ellipse(aes(label = .data[[col_variable]], filter = !is.na(.data[[col_variable]])),
      #                   expand = 0.02) +
      scale_colour_manual(
        values = cluster_col,
        na.value = "grey50") +
...
get_genotype_labels <- function(allele_labels) {
  genotype_labels <- c(
    "AA" = paste0(allele_labels[c("A", "A")], collapse = "/"),
    "AB" = paste0(allele_labels[c("A", "B")], collapse = "/"),
    "BB" = paste0(allele_labels[c("B", "B")], collapse = "/"))
...
plot_cnv_variant_intensities_with_different_genotype <- function(
  ...
  geno_genotype_labels <- get_genotype_labels(geno_allele_labels)
...
mean_shift_clustering <- function(principal_intensity_matrix, bandwidth = 0.20) {
  result <- ms(
...
pca_components <- function(
  converted_data_frame, number_of_pcs) {
...
main <- function(argv = NULL) {
  ...
  principal_components <- read_csv("/groups/umcg-lifelines/tmp01/projects/ov21_0355/pgx-pipeline/analyses/cyp2d6_cnv_calling/out.intensity_correction.pcs.csv.gz") %>%
  ...
  r_matrix <- read_tsv("/groups/umcg-lifelines/tmp01/projects/ov21_0355/pgx-pipeline/analyses/cyp2d6_cnv_calling/out.mat.R.csv")
  r_as_marix <- r_matrix %>% select(-variant) %>% as_matrix()
  rownames(r_as_marix) <- r_matrix$variant
  ...
  pdf(file.path(".", sprintf(
    "out.sample_pcs.%s.pdf", format(Sys.Date(), "%Y%m%d"))),
      useDingbats = FALSE, width = 8, height = 8)

  par(xpd = NA)

  ggplot(principal_components, aes(`0`, `1`)) +
    geom_point()

  ggplot(principal_components, aes(`2`, `3`)) +
    geom_point()

  ggplot(principal_components, aes(`4`, `5`)) +
    geom_point()

  ggplot(principal_components, aes(`6`, `7`)) +
    geom_point()

  dev.off()

  intensities_corrected <- read_csv("/groups/umcg-lifelines/tmp01/projects/ov21_0355/pgx-pipeline/analyses/cyp2d6_cnv_calling/out.intensity_correction.corrected.csv.gz") %>%
    rename("Sample_ID" = `Sample ID`) %>%
    pivot_longer(cols = -Sample_ID, values_to = "R_corrected", names_to = "variant")
  intensities_data_frame <- read_tsv("/groups/umcg-lifelines/tmp01/projects/ov21_0355/pgx-pipeline/analyses/cyp2d6_cnv_calling/out.intensity_data_frame.csv.gz") %>%
  ...
  variant_alleles <- as_tibble(
    str_match(unique(intensities_data_frame$SNP), "^\\[([agctdiAGCTDI]+)/([agctdiAGCTDI]+)\\]$")) %>%
    rename(c("SNP" = "V1", "A" = "V2", "B" = "V3"))
  ...
  combined_data_frame <- corrected_intensities_data_frame %>%
  ...
  intensity <- c(
    "DUP-rs1065852", "rs1065852", "1080989",
    "rs1080985", "rs1080983", "rs28360521",
    "rs1135823", "seq-rs1135824", "DUP-seq-rs1135824",
    "chr22-42525044:rs1135824", "seq-rs5030867", "rs72549349",
    "rs1135740", "DUP-rs1135840")

  # converted_data_frame <- combined_data_frame %>%
  #   filter(variant %in% vars) %>%
  #   pivot_longer(cols = c(X_corrected, Y_corrected), names_to = "signal_type", values_to = "value_corrected") %>%
  #   rowwise() %>%
  #   mutate(
  #     variant_signal_label = case_when(
  #       signal_type == "X_corrected" ~ paste(variant, A, sep = "_"),
  #       signal_type == "Y_corrected" ~ paste(variant, B, sep = "_"))) %>%
  #   ungroup() %>%
  #   pivot_wider(id_cols = "Sample_ID",
  #               names_from = "variant_signal_label",
  #               values_from = "value_corrected")


  # clustered_variant_tibble <- glued_variant_names %>%
  #   distinct(variant_a, type_a, var_a) %>%
  #   inner_join(tibble(cluster = sub_grp, var_a = names(sub_grp))) %>%
  #   rename_at(.vars = vars(ends_with("_a")),
  #             .funs = funs(sub("[_]a$", "", .)))
  #
  # cluster_of_interest <- clustered_variant_tibble %>%
  #   filter(variant == "rs28360521", type == "R_corrected") %>%
  #   pull(cluster) %>% .[[1]]

  converted_data_frame <- combined_data_frame %>%
    pivot_longer(c(R_corrected, theta), names_to = "type", values_to = "value", values_drop_na = T) %>%
    mutate(var = str_glue("{variant}_type_{type}", variant = variant, type = type)) %>%
    #inner_join(clustered_variant_tibble, by = c("variant", "type")) %>%
    filter(variant %in% intensity, type == "R_corrected") %>%
    pivot_wider(id_cols = "Sample_ID",
                names_from = "var",
                values_from = "value")

  pca_result <- pca_components(converted_data_frame, 16)
  principal_intensity_matrix <- pca_result$x

  pc_intensities <- as_tibble(
    principal_intensity_matrix,
    rownames = "Sample_ID") %>%
    inner_join(converted_data_frame, by = "Sample_ID")

  plot_principal_component_correlations(pc_intensities)

  lpcm_results <- mean_shift_clustering(principal_intensity_matrix[,paste0("PC", c(1:2))], bandwidth = 0.12)
  ...
  cluster_col <- colorRampPalette(brewer.pal(n = 8, name = 'Set2'))(length(cluster_names))
  ...
  png_file_list <- base::lapply(combined_data_frame %>%
                                         ...
                                         distinct(variant) %>%
                                         arrange(variant) %>%
                                         pull(variant), function(var) {
    png_file_name <- file.path(".", sprintf(
      "out.intensity_variants_%s_%s.png", var, format(Sys.Date(), "%Y%m%d")))

    plot_orig <- plot_cnv_variant_intensities(
      combined_data_frame, x_variable = "X", y_variable = "Y", col_variable = "genotype", var)
    plot_corrected <- plot_cnv_variant_intensities(
      combined_data_frame, x_variable = "X_corrected", y_variable = "Y_corrected", col_variable = "genotype", var)

    png(png_file_name, width = 1080, height = 540, res = 150)
